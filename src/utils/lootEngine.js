...
export async function rollRarity(playerRank) {
  const { data } = await supabase.from('loot_drop_rates').select('*').eq('rank_name', playerRank).single();
  const rates = data || { common_chance: 85, rare_chance: 10, epic_chance: 4, legendary_chance: 1 };
  
  const roll = getRandomInt(1, 100);
  
  if (roll <= rates.legendary_chance) return 'Lendário';
  if (roll <= rates.legendary_chance + rates.epic_chance) return 'Épico';
  if (roll <= rates.legendary_chance + rates.epic_chance + rates.rare_chance) return 'Raro';
  return 'Comum';
}
...
export async function generateLootStats(rarity, playerRank, ninjaClass = '') {
  ...
  const generateValue = (statName) => {
    ...
    let val = getRandomInt(statDef.min_value, statDef.max_value);
