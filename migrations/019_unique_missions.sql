-- migrations/019_unique_missions.sql
...
BEGIN
    ...
    -- Update completed_missions
    v_completed_missions := COALESCE(v_player.completed_missions, '[]'::jsonb);
    -- Check if it's already there to avoid duplicates (just in case)
    IF NOT v_completed_missions @> to_jsonb(p_mission_id) THEN
        v_completed_missions := v_completed_missions || to_jsonb(p_mission_id);
    END IF;
